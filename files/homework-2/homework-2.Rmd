...
```{r Second Hand House Sales in Time, out.width= '50%'}
...
```{r Distribution of Second Hand House Sales in Turkey, out.width= '50%'}
...
Adding months as a factor variable clearly improved my model. The autocorrelation in month nine is obeserve as a significant variable in seasonality. 
...
Removing outliers improved my model significantly and decrease the increasing variance in residuals. 
...
It is expected that negative realtionship between interest rates and second hand house sales since it is long term dept. Negative correlation rate is support this idea. 
```{r}
...
sales$fitted <- fit$fitted
sales[,time:= c(1:92)]
ggplot(sales, aes(x = time)) + geom_line(aes(y= fitted, color = "fitted")) + geom_line(aes(y= log_sales, color = "sales")) +
  labs(
    title = "Fitted and Sales",
    x = "time",
    y= "log(sales)"
  ) +
  theme_minimal()
...
In the end, my model is insufficient for explain variance in the data. Expecially in lower values since there is more deviations observed in "Fitted vs Sales", besides even if my residuals have nearly zero mean, they have high variance. 
...
```{r prediction}
sales= rbind(sales, list(0,11.26287,0,4,NA,99,0,0,0,93))
exp(predict(fit,sales[is.na(fitted)==T]))

```

There is a predictions and actual values of the second hand house sales in Turkey based on my model. It is expected an increase. 
...
# Conclusion
 In this report, I explained my first time series regression model for the forecast next month's second hand house sales. My model is not a strong model since its Adjusted R-squared value is low and there is still variability in my model. 
 First of all, I take the logarithm of sales since there is a increasing variance, then I tried to add time series components to my model. All components was significant and I kept them in my model.
 Before add new variablesi I removed outliers from my data. 
 Finally, I analyzed the relationship between the second hand house sales and some economic factors whİch I get from [EVDS](https://evds2.tcmb.gov.tr/index.php?/evds/serieMarket). Unfortunately only interest of consumer loans was significant, and I keep it in my model only interest rate of consumer loans. 
 In the End, I compare my predictions with actual variables, and predict next month sale. 

# References 
[the Central Bank of the Republic of Turkey’s Electronic Data Delivery System](https://evds2.tcmb.gov.tr)

[PS4](https://moodle.boun.edu.tr/pluginfile.php/788893/mod_resource/content/1/time_series_regression.R)

# RMD 
 To understand better my model, click [here](https://github.com/BU-IE-360/spring21-seymacakir/blob/master/files/homework-2/homework-2.Rmd) for my RMD file. 
